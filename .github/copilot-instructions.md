...
### Form submit flow
`Form.onSubmit` receives a submit context with `event`, `validate`, and `commit`. Call `validate({ focusFirstError: true, scrollOffset: 100 })` to run all registered field validators, then call `commit()` to apply and optionally focus the first error.
